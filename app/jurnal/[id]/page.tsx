    ...
    return (
      <div className="page-shell grid min-h-screen place-items-center pt-24 text-center">
        <div>
          ...
          <Link href="/jurnal" className="mt-5 inline-block text-sm font-bold text-lime-300">Back to journal</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-4xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link href="/jurnal" className="group inline-flex items-center gap-2 text-xs font-bold text-zinc-500 transition-colors hover:text-white">
            <span className="transition-transform group-hover:-translate-x-1">←</span> All journal entries
          </Link>
        ...
        <header className="pb-12 pt-14 text-center">
          ...
          <div className="mx-auto mt-9 flex w-max items-center gap-3">
            ...
            <span className="text-left">
              ...
              <span className="block text-[10px] text-zinc-600">Informatics Student</span>
            </span>
          </div>
        </header>
        ...
        <article className="glass-card rounded-[1.7rem] p-6 md:p-12">
          <div
            ...
            dangerouslySetInnerHTML={{
              __html:
                formatJournalHtml(journal.konten) ||
                "<p class='italic text-zinc-600'>This journal entry does not have any content yet.</p>",
            }}
          />
        </article>
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-5 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-6 text-center sm:flex-row sm:text-left">
          <div>
            <p className="text-sm font-bold text-white">Found something interesting?</p>
            <p className="mt-1 text-xs text-zinc-600">Share this note with someone who might enjoy it.</p>
          </div>
          <ShareButton path={`/jurnal/${journal.id}`} title={localizeJournalTitle(journal.judul)} />
        </div>
      </div>
